...
*/
    if(userInput==''||userInput.trim==''){
        ...
        session.send("Hello User,How can i help you today");
         ...
         session.send("- Manager Level");
         session.send("- Engineer Level");
        ...
        session.send("- HR Manager - HM1098");
        session.send("- Engineering Manager - EM1099")
        ...
        session.send("Following jobs are available");
        session.send("- Software Engineer - SE2011");
        session.send("- Senior Software Engineer - SSE2012");
        ...
        session.send("- Compensation and Benefits");
        session.send("- Recruitment and Hiring");
        session.send("- Performance/Employee Evaluation");
        session.send("- Training and Staff Development")
        ...
        session.send("- Project management");
        session.send("- Technical understanding");
        session.send("- Performance/Employee Evaluation");
        session.send("- Management duties");
        session.send("Type apply in case you want to apply for this job");
    }
        ...
        session.send("- Software Development");
        session.send("- Understanding of Frameworks such as Spring and Hibernate");
        session.send("- Coding Abilities");
        session.send("- Algorithms and Data Structures");
        session.send("Type apply in case you want to apply for this job");
    }
    ...
    {   
        ...
        if(userInput>10){
            session.send("What is your qualification");
            session.send("- Graduate");
